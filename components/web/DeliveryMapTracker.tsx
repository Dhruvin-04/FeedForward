...
import { X, Store, Building2, Bike } from 'lucide-react'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden mx-4">
        ...
        {/* Volunteer Info */}
        {volunteerName && (
          <div className="px-4 py-3 border-t border-b bg-gray-50 flex flex-wrap gap-4 text-sm">
            <span className="flex items-center gap-1.5">
              <Bike className="w-4 h-4 text-green-500" />
              <span className="font-medium">{volunteerName}</span>
            </span>
            {volunteerPhone && (
              <span className="text-gray-600">{volunteerPhone}</span>
            )}
            {pickupCode && (
              <span className="text-gray-600">
                Code: <span className="font-mono font-bold text-primary">{pickupCode}</span>
              </span>
            )}
          </div>
        )}
